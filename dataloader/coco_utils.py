#참고 코드 https://github.com/fitexmage/XNect_Implementation/blob/master/data_process/coco_process_utils.py
...
#annotation 체크 (keypoint, area, iscrowd 체크)
...
def get_heatmap(coco , img, keypoints, sigma):
    ...
    for person_id in range(keypoints.shape[0]):
        ...
        for i in range(keypoints.shape[1]):
            keypoints = keypoints_person[i]
            if keypoints[2] > 0:
                out_map[i] = np.maximum(out_map[i], DrawGaussian(out_map[i], keypoints[0:2], sigma=sigma))
    out_map[n_joints] = 1 - np.sum(out_map[0:n_joints], axis = 0)
    return out_map
...
def get_paf(coco, img, keypoints, sigma_paf, variable_width):
    ...
    for person_id in range(keypoints.shape[0]):
        ...
        for i in range(len(BODY_PARTS)):
            part = BODY_PARTS[i]
            keypoints_1 = keypoints_person[part[0], :2]
            keypoints_2 = keypoints_person[part[1], :2]
            if keypoints_person[part[0], 2] > 0 and keypoints_person[part[1], 2] > 0:
                part_line_segment = keypoints_2 - keypoints_1
                ...
                if l > 1e-2:
                    ...
                    dist_along_part = v[0] * (x - keypoints_1[0]) + v[1] * (y - keypoints_1[1])
                    dist_per_part = np.abs(v_per[0] * (x - keypoints_1[0]) + v_per[1] * (y - keypoints_1[1]))
                    mask1 = dist_along_part >= 0
                    mask2 = dist_along_part <= 1
                    mask3 = dist_per_part <= sigma 
                    mask = mask1 & mask2 & mask3
                    out_pafs[i, 0] = out_pafs[i, 0] + mask.astype('float32') * v[0]
                    out_pafs[i, 0] = out_pafs[i, 1] + mask.astype('float32') * v[1]
...
def get_ignore_mask(coco, img, annots):
    ...
    masks = []
    for annot in annots:
       mask = coco.annToMaks(annot).astype('bool')
       masks.append(mask)
...
def clean_annot(coco, data_path, split):
    ...
    if os.path.exists(ids_path):
        print('Loding ... filtered annotations for {} from {}'.format(split, ids_path))
        with open(ids_path, 'rb') as f:
            return pickle.load(f)

    else:
        print('Filtering annotations for {}'.format(split))
        person_ids = coco.getCatIDs(catIds = person_ids)
